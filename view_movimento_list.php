						...
						<table cellpadding="0" cellspacing="0" width="100%" class="sortable">
							<thead>
								...
								foreach($campos as $titulo=>$campo){
									...
									$odescendente = 'DESC';
									if(!empty($vetorclassificacao)){
										if(isset($vetorclassificacao[$campo])){
							...
							?>
								...
								foreach($campos as $titulo=>$campo){
									...
									if($campo=='status'){
										...
										if($campo=='qtd_militar' || $campo=='qtd_diaria' || $campo=='adicional' || $campo=='valor_diaria'|| $campo=='valor_passagem'){
											...
											echo "<td><p style='font-size:9px;padding-bottom:2px;color:black;min-height:15px;' id='{$campo}p{$resultados['id']}' onclick=\"{$funcoes[$campo]}('{$campo}','{$resultados['id']}','{$controllernome}','{$tipos[$campo]}','$completafuncao');\"  >";
											if(!empty($resultados[$campo])){ echo  $resultados[$campo]; }
											echo "</p></td>";
										}else{
												echo "<td><p style='font-size:9px;padding-bottom:2px;color:black;min-height:15px;' id='{$campo}p{$resultados['id']}' onclick=\"{$funcoes[$campo]}('{$campo}','{$resultados['id']}','{$controllernome}','{$tipos[$campo]}','$completafuncao');\" >";
											if(!empty($resultados[$campo])){ 
												echo  $resultados[$campo]; 
											}
												echo "</p></td>";
								...
								?>
									...
									<!-- -->
									<td class="acoes">&nbsp;</td>
									<td class="acoes"><a href="javascript:exclui('<?php echo $resultados['id']; ?>', '<?php echo $resultados['data'].'->('.$resultados['proced'].'-'.$resultados['destino']; ?>)','#del<?php echo $resultados['id']; ?>');" title="Excluir" id='del<?php echo $resultados['id']; ?>'><img src="images/lixo.png" alt="Excluir"></a></td>
